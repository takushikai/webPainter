...
(function()
{
    if(KTJwebDrawUsing == true){
        console.log("既に起動されています。");
    }
    else{
    ...
    //     drawOrErase(event.pageX, event.pageY);
    // });
    // canvas.addEventListener("touchmove", (event) => {
    //     drawOrErase(event.changedTouches[0].pageX, event.changedTouches[0].pageY);
    // });
    ...
    //バニシングオンオフ
    pencil.addEventListener("click", setVanishing);
...
var KTJwebDrawUsing = true;//const,letだとなぜかエラー